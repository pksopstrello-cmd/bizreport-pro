...
def compute_daily_summary(df, merchant_filter, tx_type_filter):
      """
          Compute day-by-day summary table matching the JP-JWINR Daily Summary format.
              Returns a list of row dicts or None.
                  """
    d = df.copy()
    ...
    if col_rdp_amt:
        ...
        d["_rdp_amt"] = d["_is_manual"] * d["_amount"]
    ...
    for date_val in sorted(d["_date"].unique()):
        ...
        rows.append({
                      ...
                      # raw for totals
                      "_auto_amt": auto_amt,
                      "_auto_count": auto_count,
                      "_mdp_amt": mdp_amt,
                      "_mdp_count": mdp_count,
                      "_net_amt": net_amt,
                      "_fail_amt": fail_amt,
                      "_fail_count": fail_count,
                      "_total_count": total_count,
                      "_total_success": int(total_success),
                      "_auto_success": int(auto_success),
                      "_rdp_count": rdp_count,
                      "_rdp_amt": rdp_amt,
        })
    ...
    # Remove raw columns for display
    display_rows = []
    for r in rows:
              dr = {k: v for k, v in r.items() if not k.startswith("_")}
...
def show_daily():
    ...
    if col_date:
            ...
            if len(dates) > 0:
                              d_min = dates.min().strftime("%B %d")
                d_max = dates.max().strftime("%d")
    ...
    for i, (mf, tx_type, title) in enumerate(configs):
        with tabs[i]:
            ...
            if result:
                ...
                st.warning(f"No {tx_type} data found. Checking available columns...")
